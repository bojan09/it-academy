  ...
  return (
    ...
    >
      ...
      <section>
        ...
        <CodeBlock title="Disk inspection commands" language="bash" code={`# ── Block device tree ───────────────────────────────────────
...
sdb           8:16    20G   disk             ← Unpartitioned new disk`}</pre>
      ...
      <section>
        ...
        <CodeBlock title="Full workflow: new disk → mounted filesystem" language="bash" code={`# ═══ STEP 1: Create partition ════════════════════════════════
...
df -h /mnt/datastore      # Confirm still works`} />
      ...
      <section>
        ...
        <CodeBlock className="mt-5" title="LVM setup — full workflow" language="bash" code={`# ═══ STEP 1: Initialise physical volumes ═════════════════════
...
echo "/dev/vgdata/lvlogs /var/log/app xfs   defaults  0 2" | sudo tee -a /etc/fstab`} />
        ...
        <CodeBlock language="bash" code={`# Add a new physical disk to an existing VG
...
sudo fdisk -l /dev/sdc`}
              output={`NAME   MAJ:MIN  SIZE  TYPE  MOUNTPOINT
sda      8:0     40G   disk
├─sda1   8:1      1G   part  /boot
└─sda2   8:2     39G   part
...
sdc      8:32    10G   disk             ← new disk 2`}
            ...
            <LabStep number={2}
              ...
              command={`sudo apt install lvm2 -y
...
sudo lvs`}
  ...
  lvlogs vglab -wi-a----- 5.00g`}
            ...
            <LabStep number={4}
              ...
              command={`sudo mkfs.ext4 -L "labdata" /dev/vglab/lvdata
...
/dev/mapper/vglab-lvlogs    5.0G   68M  5.0G   2% /mnt/lablogs`}
            ...
            <LabStep number={5}
              ...
              command={`# Extend LV + filesystem in one step (-r flag)
...
/dev/mapper/vglab-lvdata     11G   24M   10G   1% /mnt/labdata  ← now 11GB!
  ...
  lvdata vglab -wi-ao---- 11.00g  ← extended successfully`}
      ...
      <section>
        ...
        <CodeBlock title="Disk & LVM command cheat sheet" language="bash" code={`# ── Inspection ──────────────────────────────────────────────
