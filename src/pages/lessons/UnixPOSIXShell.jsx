    ...
    >
      ...
      <section>
        ...
        <CodeBlock title="production-ready POSIX sh template" language="bash"
          code={["#!/bin/sh","# POSIX-compliant script","set -euo pipefail","","SCRIPT_DIR=\"$(cd \"$(dirname \"$0\")\" && pwd)\"","SCRIPT_NAME=\"$(basename \"$0\")\"","LOG_FILE=\"/var/log/${SCRIPT_NAME%.sh}.log\"","LOCK_FILE=\"/tmp/${SCRIPT_NAME%.sh}.lock\"","","log() { echo \"[$(date '+%Y-%m-%d %H:%M:%S')] $*\" | tee -a \"$LOG_FILE\"; }","die() { log \"ERROR: $*\" >&2; exit 1; }","","# Cleanup on exit","cleanup() {","    rm -f \"$LOCK_FILE\"","    log 'Script finished'","}","trap cleanup EXIT INT TERM","","# Prevent concurrent runs","[ -f \"$LOCK_FILE\" ] && die 'Already running'","echo $$ > \"$LOCK_FILE\"","","# ── Main logic ────────────────────────────────────────────","log 'Script started'","","[ -d /var/backup ] || mkdir -p /var/backup","","for service in ssh nginx; do","    if command -v \"$service\" >/dev/null 2>&1; then","        log \"Found: $service\"","    else","        log \"Not found: $service\"","    fi","done"].join('\n')} />
      </section>
      <section>
        ...
        <div className="lab-block">
          ...
          <div className="lab-body space-y-8">
            <LabStep number={1} description="Verify your script runs under dash (strict POSIX sh) not just bash."
              command={["# Check which shell /bin/sh points to","ls -la /bin/sh","","# Run a script under dash explicitly to test portability","dash myscript.sh","","# Check for bashisms that break portability","shellcheck --shell=sh myscript.sh"].join('\n')}
